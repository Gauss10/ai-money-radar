...
async def main():
    ...
    if run_all or args.twitter_only:
        ...
        if not twitter_feed.get("x") and "TWITTER_COOKIES not set" in (twitter_feed.get("errors") or []):
            existing_twitter = load_feed("feed-x.json")
            if existing_twitter and existing_twitter.get("x"):
                log("ℹ️  Twitter skipped; keeping existing feed-x.json cache")
                twitter_feed = existing_twitter
